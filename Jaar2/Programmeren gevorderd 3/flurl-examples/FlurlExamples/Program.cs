...
namespace FlurlExamples
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                var newRepository = flurlRequestHandler.CreateRepository("MyDemoRepo", "Test-Repository").Result;
            ...
            var editedRepository = flurlRequestHandler.EditRepository("MyDemoRepo", "Test-Repository").Result;
            WriteInColor("Repository edited", ConsoleColor.Green);
            WriteResult(editedRepository);
            Console.WriteLine();

            var deleteRepoResult = flurlRequestHandler.DeleteRepository("MyDemoRepo", "Test-Repository").Result;
